...
describe('App', () => {
  ...
  it('When delete is clicked, the associated card will delete from the BE and DOM', async () => {
		...
		const urlCard1Before = await waitFor(() => getByText('Awesome photo'));
		expect(urlCard1Before).toBeInTheDocument();
		...
		const urlCard1After = await waitFor(() => getByText('Awesome photo'));

		await waitFor(() => {
			expect(urlCard1After).not.toBeInTheDocument();
		})
	});
})